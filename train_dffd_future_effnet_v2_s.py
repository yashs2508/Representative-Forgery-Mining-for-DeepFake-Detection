import torch
import torchvision
from utilscopyeffnetv2s.utils import data_prefetcher_two, cal_fam, setup_seed, calRes
import utilscopyeffnetv2s.datasets_profiles as dp
...
parser.add_argument('--modelname', default="efficientnet_v2_s", type=str)
...
def Log(log):
    print(log)
    path = "logs/croppedimgs/logs_dffd_future_effnet_v2_s/"
    if os.path.exists(path):
        pass
    else:
        os.makedirs(path)
    
    with open("logs/croppedimgs/logs_dffd_future_effnet_v2_s/"+upper+"_"+modelname+".log", "a") as f:
        f.write(log+"\n")
        f.close()
...
if __name__ == "__main__":
    ...
    model = torchvision.models.efficientnet.efficientnet_v2_s().cuda()
        ...
        while data is not None and batchind < args.max_batch:
            ...
            if batchind % args.logbatch == 0:
                ...
                if batchind % args.savebatch == 0 or TPR_4 > MAX_TPR_4:
                    MAX_TPR_4 = TPR_4
                    if args.save_model:
                        os.makedirs('models/croppedimgs/models_dffd_future_effnet_v2_s', exist_ok=True)
                        torch.save(model.state_dict(), "models/croppedimgs/models_dffd_future_effnet_v2_s/" + upper+"_"+modelname+"_model_batch_"+str(batchind))
                    if args.save_optim:
                        os.makedirs('models/croppedimgs/models_dffd_future_effnet_v2_s', exist_ok=True)
                        torch.save(optim.state_dict(), "models/croppedimgs/models_dffd_future_effnet_v2_s/" + upper+"_"+modelname+"_optim_batch_"+str(batchind))
