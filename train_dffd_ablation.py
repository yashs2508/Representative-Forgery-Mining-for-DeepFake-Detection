import torch
from utilscopyablation.utils import data_prefetcher_two, cal_fam, setup_seed, calRes
from pretrainedmodels import xception
import utilscopyablation.datasets_profiles as dp
...
def Log(log):
    print(log)
    path = "logs/croppedimgs/logs_dffd_ablation/wFAM/"
    if os.path.exists(path):
        pass
    else:
        os.makedirs(path)
    
    with open("logs/croppedimgs/logs_dffd_ablation/wFAM/"+upper+"_"+modelname+".log", "a") as f:
        f.write(log+"\n")
        f.close()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        while data is not None and batchind < args.max_batch:
            ...
            # print('fam shape', mask.shape)
            # print('fam 1 shape', mask[1].shape)
            ...
            for i in range(len(mask)):
                maxind = np.argsort(mask[i].cpu().numpy().flatten())[::-1][:1]
                # print('maxind', maxind)
                # print('shape maxind', maxind.shape)
                # print()
            ...
            if batchind % args.logbatch == 0:
                ...
                if batchind % args.savebatch == 0 or TPR_4 > MAX_TPR_4:
                    MAX_TPR_4 = TPR_4
                    if args.save_model:
                        os.makedirs('models/croppedimgs/models_dffd_ablation/wFAM/', exist_ok=True)
                        torch.save(model.state_dict(), "models/croppedimgs/models_dffd_ablation/wFAM/" + upper+"_"+modelname+"_model_batch_"+str(batchind))
                    if args.save_optim:
                        os.makedirs('models/croppedimgs/models_dffd_ablation/wFAM/', exist_ok=True)
                        torch.save(optim.state_dict(), "models/croppedimgs/models_dffd_ablation/wFAM/" + upper+"_"+modelname+"_optim_batch_"+str(batchind))
