...
def Log(log):
    print(log)
    path = "logs/croppedimgs/logs_dffd_future_gaussian/"
    if os.path.exists(path):
        pass
    else:
        os.makedirs(path)
    
    with open("logs/croppedimgs/logs_dffd_future_gaussian/"+upper+"_"+modelname+".log", "a") as f:
        f.write(log+"\n")
        f.close()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        while data is not None and batchind < args.max_batch:
            ...
            ''' ↓ the implementation of new RFM ↓ '''
            # getting gaussian of each images in the batch 'data'
            # since data is of tensor type so 1st change it to array
            datacopy = data.clone()
            dataarray = datacopy.cpu().numpy()  # converting torch to nd array
            gaussian_data = torch.zeros_like(datacopy)

            for i in range(len(data)):
                imgg = dataarray[i]
                gauss = cv2.GaussianBlur(imgg,(5,5),0,borderType= cv2.BORDER_REFLECT)
                gauss = 2*((gauss - np.min(gauss))/np.ptp(gauss))-1  # Normalised [-1,1]
                gaussian_data[i][:] = torch.from_numpy(gauss) # converting nd array to torch
            ...
            data = imgmask * data + (1-imgmask) * gaussian_data
            ''' ↑ the implementation of new RFM ↑ '''
            ...
            if batchind % args.logbatch == 0:
                ...
                if batchind % args.savebatch == 0 or TPR_4 > MAX_TPR_4:
                    MAX_TPR_4 = TPR_4
                    if args.save_model:
                        os.makedirs('models/croppedimgs/models_dffd_future_gaussian', exist_ok=True)
                        torch.save(model.state_dict(), "models/croppedimgs/models_dffd_future_gaussian/" + upper+"_"+modelname+"_model_batch_"+str(batchind))
                    if args.save_optim:
                        os.makedirs('models/croppedimgs/models_dffd_future_gaussian', exist_ok=True)
                        torch.save(optim.state_dict(), "models/croppedimgs/models_dffd_future_gaussian/" + upper+"_"+modelname+"_optim_batch_"+str(batchind))
