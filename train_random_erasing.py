...
def Log(log):
    print(log)
    path = "logs/croppedimgs/logs_random_erasing/dffd_grpA"
    if os.path.exists(path):
        pass
    else:
        os.makedirs(path)
    
    with open("logs/croppedimgs/logs_random_erasing/dffd_grpA"+upper+"_"+modelname+".log", "a") as f:
        f.write(log+"\n")
        f.close()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        while data is not None and batchind < args.max_batch:
            ...
            model.eval()
            imgmask = torch.ones_like(data)
            sl, sh = 0.02, 0.4    # Se/S is in range sl(minimum) and sh(maximum)
            r1 = 0.3  # aspect ratio re of erasing rectangle region(Ie) is randomly initialized between r1 and r2
            r2 = 1/r1
            imgh = imgmask[0][0].shape[0]
            imgw = imgmask[0][0].shape[1]
            S = imgh * imgw # area of the original image

            for i in range(len(imgmask)): 
                Se = random.uniform(sl, sh) * S      # area of erasing rectangle region
                re = random.uniform(r1, r2)          # aspect ratio    
                h = int(round(np.sqrt(Se*re)))       # height of Ie (erasing rectangle region)
                w = int(round(np.sqrt(Se/re)))       # width of Ie
                
                # Ie = (xe, ye, xe+he, ye+we)
                top = random.randint(0, imgh)  # pixel x-coordinate of Ie,i.e xe
                left = random.randint(0, imgw)  # pixel y-coordinate of Ie,i.e ye
                bot = min(top+h, imgh)
                right = min(bot+w, imgw)
                
                imgmask[i][:, top:bot, left:right] = torch.zeros_like(imgmask[i][:, top:bot, left:right])
            ...
            if batchind % args.logbatch == 0:
                ...
                if batchind % args.savebatch == 0 or TPR_4 > MAX_TPR_4:
                    MAX_TPR_4 = TPR_4
                    if args.save_model:
                        os.makedirs('models/croppedimgs/models_random_erasing/dffd_grpA', exist_ok=True)
                        torch.save(model.state_dict(), "models/croppedimgs/models_random_erasing/dffd_grpA/" + upper+"_"+modelname+"_model_batch_"+str(batchind))
                    if args.save_optim:
                        os.makedirs('models/croppedimgs/models_random_erasing/dffd_grpA', exist_ok=True)
                        torch.save(optim.state_dict(), "models/croppedimgs/models_random_erasing/dffd_grpA/" + upper+"_"+modelname+"_optim_batch_"+str(batchind))
