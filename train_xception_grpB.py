...
def Log(log):
    print(log)
    path = "logs/croppedimgs/logs_xception_grpB/"
    if os.path.exists(path):
        pass
    else:
        os.makedirs(path)
    
    with open("logs/croppedimgs/logs_xception_grpB/"+upper+"_"+modelname+".log", "a") as f:
        f.write(log+"\n")
        f.close()
        ...
        while data is not None and batchind < args.max_batch:
            ...
            if batchind % args.logbatch == 0:
                ...
                if batchind % args.savebatch == 0 or TPR_4 > MAX_TPR_4:
                    MAX_TPR_4 = TPR_4
                    if args.save_model:
                        os.makedirs('models/croppedimgs/models_xception_grpB', exist_ok=True)
                        torch.save(model.state_dict(), "models/croppedimgs/models_xception_grpB/" + upper+"_"+modelname+"_model_batch_"+str(batchind))
                    if args.save_optim:
                        os.makedirs('models/croppedimgs/models_xception_grpB', exist_ok=True)
                        torch.save(optim.state_dict(), "models/croppedimgs/models_xception_grpB/" + upper+"_"+modelname+"_optim_batch_"+str(batchind))
